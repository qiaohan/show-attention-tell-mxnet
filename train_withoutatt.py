...
def main():
    ...
    model = CaptionGenerator(word_to_idx, dim_feature=[196, 512], dim_embed=512, batch_size=batch_size,
                                       dim_hidden=512, n_time_step=max_len+1, prev2out=True, ctx2out=True, dropout=True, ctx=mx.gpu(0))
    ...
    solver = CaptioningSolver(model, data, val_data, n_epochs=50000, batch_size=batch_size, update_rule='adam',
                                          learning_rate=0.005, print_every=50, save_every=1, image_path='./image/',
